...
def download_file(request):
    ...
    if file_type == "abstract":
        path_to_file = "abstracts/" + file_name
    else:
        path_to_file = "papers/" + file_name
...
def add_conference_handle(request):
    if request.method == "POST":
        ...
        confs = models.Conference.objects.all()
        time_now = timezone.now()
        return render(request,"conferences.html",{"islogged_in":islogged_in,
                                                  "confs":confs,
                                                  "user_type":request.COOKIES.get('user_type'),
                                                  "is_accepted": is_accepted,
    ...
    if request.method == "GET":
        ...
        if is_admin_logged_in:
            ...
            if user_type == "reviewer":
                ...
                reviewer.save()
                if is_accepted:
                    message = "You have been deauthorised in Sabanci Conference System as a reviewer."
                else:
                    message = "You have been authorised in Sabanci Conference System as a reviewer."
            elif user_type == "chair":
                chair = models.Chair.objects.get(email=user_mail)
                chair.is_accepted = (not is_accepted)
                chair.save()
                if is_accepted:
                    message = "You have been deauthorised in Sabanci Conference System as a chair."
                else:
                    message = "You have been authorised in Sabanci Conference System as a chair."
            email_send_thread = Thread(target=Utils.send_email, args=("Your status in Sabanci conference system.",
                                                                                       message,
                                                                                       user_mail,
                                                                                       email_lock
                                                                                       ))
